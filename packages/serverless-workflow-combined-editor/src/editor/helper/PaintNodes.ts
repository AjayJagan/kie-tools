...
const paintCompletedNode = async (node: Node) => {
  await window.editor.canvas.setBackgroundColor(node.uuid, "lightgrey");
};

const isPointingToAnyCompletedNode = (completedNodes: Node[], node: Node) => {
  return (
    node.outEdges.filter((edge) => completedNodes.filter((node) => edge.target === node.uuid).length > 0).length > 0
  );
};

export const paintCompletedNodes = (nodesToColor: string[], isWorkflowCompleted: boolean) => {
  Promise.all(nodesToColor.map((name) => window.editor.session.getNodeByName(name)))
    .then((completedNodes) =>
